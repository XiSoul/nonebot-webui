...
const extensionUninstallConfirmModal = ref<HTMLDialogElement>()
const pypiData = ref<PypiInfo>()
...
const installModule = async (module: T) => {
  ...
  const { data, error } = await StoreService.installNonebotModuleV1StoreNonebotInstallPost({
    ...
    // TODO: Module 和 Plugin 的联合类型仍有冲突，后续再整理
    // @ts-ignore
    body: {
      ...module,
  ...
  if (error) {
    installShouldRestart.value = false
    toast.add('error', `提交安装失败，原因：${getErrorMessage(error)}`, '', 5000)
    return
  }
...
const retryInstall = () => {
  void installModule(props.data)
}

const handleInstallFinished = async () => {
  if (!nonebotStore.selectedBot) return
  await nonebotStore.loadBots()
  await store.updateData(nonebotStore.selectedBot.project_id, false)
  toast.add(
    'success',
    installShouldRestart.value ? '安装成功，实例已自动完成停机安装并重新启动' : '安装成功',
    '',
    4000
  )
  installShouldRestart.value = false
...
const getUpdateTime = computed(() => {
  if (!('time' in props.data)) return '未知'
  ...
  if (diffYears >= 1) return `${Math.floor(diffYears)}年前`
  if (diffMonths >= 1) return `${Math.floor(diffMonths)}个月前`
  if (diffDays >= 1) return `${Math.floor(diffDays)}天前`
  if (diffHours >= 1) return `${Math.floor(diffHours)}小时前`
  return '刚刚'
})
</script>
...
<template>
  <LogView
    ...
    @retry="retryInstall"
    @finished="handleInstallFinished"
  />

  <dialog ref="extensionUninstallConfirmModal" class="modal">
    <div class="modal-box rounded-lg flex flex-col gap-4">
      <h3 class="font-semibold text-lg">确认卸载</h3>

      <div v-if="isModuleActionLocked" class="alert alert-warning text-sm">
        当前实例正在运行，卸载仍需先停止实例。
      </div>

      <div class="w-full grid grid-cols-2 gap-4">
        <button class="btn btn-sm" :disabled="isModuleActionLocked" @click="uninstallModule(props.data)">
          确定
        </button>
  ...
  <dialog ref="extensionCardModal" class="modal">
    <div class="modal-box rounded-lg flex flex-col gap-4">
      ...
      >
        ...
        <div class="flex items-center gap-2">
          ...
          <button
            v-else
            class="btn btn-sm"
            @click="installModule(props.data)"
          >
            安装
          </button>
        </div>
      ...
      <div v-if="isModuleActionLocked" class="alert alert-warning text-sm">
        当前实例正在运行，点击安装后会先停止实例，安装完成再自动启动；卸载仍需先停止实例。
      </div>
