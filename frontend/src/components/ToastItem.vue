  ...
  <div class="p-4 bg-base-100 shadow-xl rounded-lg max-w-96 min-w-60 flex flex-col gap-2">
    <div class="flex gap-2 justify-between">
      <div class="flex gap-2 text-wrap">
        <span v-if="data.type === 'success'" class="material-symbols-outlined text-success">
          check_circle
        </span>
        <span v-else-if="data.type === 'error'" class="material-symbols-outlined text-error">
          cancel
        </span>
        <span v-else-if="data.type === 'info'" class="material-symbols-outlined text-info">
          info
        </span>
        <span v-else-if="data.type === 'warning'" class="material-symbols-outlined text-warning">
          error
        </span>
